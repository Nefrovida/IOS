...
//  NefrovidaApp
//
//  Created by Leonardo Cervantes on 2/12/25.
//
            ...
            VStack(spacing: 12) {
                ...
                Button(action: {
                    Task {
                        ...
                        if success {
                            print("✅ New appointment confirmed successfully")
                            print("🔄 Attempting to cancel old appointment: \(appointmentToCancel.patientAppointmentId)")
                            
                            // Cancel the old appointment
                            let cancelled = await cancelVM.cancelSpecificAppointment(appointmentToCancel)
                            print("✅ Cancellation result: \(cancelled)")
                            
                            showSuccessAlert = true
                        } else {
                            print("❌ Failed to confirm new appointment")
                        }
                    }
                }) {
                    Text("Confirmar cita")
